...
MongoClient.connect(connectionURL, { useNewUrlParser: true }, async (error, client) =>{
    ...
    for(i=0; random.noblesNum>=i; i++){
        if(Math.random()*10<8){
            ...
            // express.listen(3443, function(){
            //     console.log('Intrigue is listening on port 3443')
            // })
});


// This was not working for me I don't know why.

// router.post('/Assasins', async(req,res)=>{
//     const Assasin={
//         name: shortName(),
//         skills:{
//             kick: Math.random()*10,
...
//     }
//     try{
//         await db.collection('Assasins').insertOne(Assasin);
//         res.status(201).send({Assasin})
//     }catch(e){
